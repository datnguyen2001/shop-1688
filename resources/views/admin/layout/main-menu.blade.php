    ...
    <ul class="sidebar-nav" id="sidebar-nav">
        @foreach(config('menu_aside.admin') as $menu)
            @if(count($menu['submenu'])>0)
                <li class="nav-item">
                    <a class="nav-link @if($menu['name'] != $page_menu) collapsed @endif"
                    ...
                    <ul id="forms-{{$menu['name']}}"
                        class="nav-content @if($menu['name'] != $page_menu) collapse @else show @endif "
                        data-bs-parent="#sidebar-nav">
                        @foreach($menu['submenu'] as $submenu)
                            <li>
                                <a @if(!empty($submenu['parameters'])) href="{{route($submenu['route'], $submenu['parameters'])}}"
                                   @else href="{{route($submenu['route'])}}"
                                   @endif class="@if($submenu['name'] == $page_sub) active @endif">
            ...
            @else
                @if(!empty($menu['route']))
                    <li class="nav-item">
                        <a class="nav-link @if($menu['name'] != $page_menu) collapsed @endif"
                           @if(!empty($menu['parameters'])) href="{{ route($menu['route'], $menu['parameters']) }}"
